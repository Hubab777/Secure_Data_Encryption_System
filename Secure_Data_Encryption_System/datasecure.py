...
if "failure_attempt" not in st.session_state:
    st.session_state.failure_attempt = 0
...
def generate_key(passkey):
    key = pbkdf2_hmac('sha256', passkey.encode(), salt, 100000, dklen=32 ) # readable bubble key        
...
# User Registration
    ...
    if st.button("Register"):
      if username and password:
        if username in stored_data:
            ...
            st.success("✅ User has registered successfully!")
    else:
    ...
    if st.button("Login"):
        if username in stored_data and stored_data[username]["password"] == hash_password(password):
            ...
            st.session_state.failure_attempt += 1
            remaining = 3 - st.session_state.failure_attempt
            st.error(f"❌❎ Invalid Credentials! Attempts left: {remaining}")
            
            if st.session_state.failure_attempt > 3:
